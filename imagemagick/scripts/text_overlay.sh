...
/usr/local/bin/convert -size 1820x \
    -background none -fill black -font "Nimbus-Sans-Bold" -pointsize 210 caption:"$text" \
    miff:- | \
/usr/local/bin/convert "$input_file" \
    ...
    miff:- | \
/usr/local/bin/convert - -size 1820x \
    -background none -fill "#fdff65" -font "Nimbus-Sans-Bold" -pointsize 210 caption:"$text" \
